...
public:
    ...
    float brake_left = 1800;
    float brake_neutral = 1300;
    float brake_right = 1900;
    int pitch = 1500; //1800
...
//        _move.type_mask = 7;
    ...
    void forward(geometry_msgs::Point goal){
        ...
        manual_move(0, pitch, 0, 000, 0, brake_neutral, key_gripper, steering_forward,0);
    }

    void turnccw(double angle){
        ROS_INFO("Sonic is moving left. Goal: %lf. Current: %lf.", angle, yaw_degrees);
        manual_move(0, pitch, 0, 000, 0, brake_neutral, brake_left, steering_left,0);
    }

    void turncw(double angle){
        ROS_INFO("Sonic is moving right. Goal: %lf. Current: %lf.", angle, yaw_degrees);
        manual_move(0, pitch, 0, 000, 0, brake_right, brake_neutral, steering_right,0);
    }
